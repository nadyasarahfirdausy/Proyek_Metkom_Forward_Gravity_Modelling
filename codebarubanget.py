...
def forward_model():
    ...
    # grid peta (bisa diubah sesuai area yang diinginkan)
    x = np.linspace(-200, 200, 150)
    y = np.linspace(-200, 200, 150)
    X, Y = np.meshgrid(x, y)
    ...
    p = ax1.contourf(X, Y, gz_mgal, levels=30)
    fig.colorbar(p, ax=ax1, label="g_z (mGal)")
    ax1.set_title("Peta Anomali Gravitasi (g_z) [mGal]")
    ...
    ax2.set_title("Profil g_z Sepanjang X (Y=0)")
    ax2.set_xlabel("X (m)")
...
# =======================================================================
root = tk.Tk()
root.title("Pemodelan Maju Gravitasi – Multi Sumber (Lat, Lon, Depth, Density + Interpolasi)")
